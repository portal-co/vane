...
type Args = { [k: string]: any; _?: string[] };
...
function parseELF(buffer: Uint8Array) {
  if (buffer.length < 16) throw new Error('File too small to be ELF');
  const EI_CLASS = buffer[4];
  const EI_DATA = buffer[5];
  if (buffer[0] !== 0x7f || buffer[1] !== 0x45 || buffer[2] !== 0x4c || buffer[3] !== 0x46) throw new Error('Not an ELF file');
  if (EI_DATA !== 1) throw new Error('Only little-endian ELF supported');
  const is64 = EI_CLASS === 2;
  if (!is64 && EI_CLASS !== 1) throw new Error('Unknown ELF class');

  const dv = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
  const readU32 = (off: number) => dv.getUint32(off, true);
  const readU16 = (off: number) => dv.getUint16(off, true);
  const readU64 = (off: number) => { const low = dv.getUint32(off, true); const high = dv.getUint32(off + 4, true); return high * 0x100000000 + low; };

  let e_phoff: number, e_phentsize: number, e_phnum: number, e_entry: number;
  if (is64) {
...
async function loadWasmPkg(pkgPath: string): Promise<any> {
...
async function main() {
  ...
  if (!input) { console.error('Missing --input <path>'); process.exit(2); }
  ...
  for (const seg of elf.segments) {
    ...
    for (let i = 0; i < fsize; i++) writeByte(vaddr + i, buf[off + i]);
    for (let i = fsize; i < msize; i++) writeByte(vaddr + i, 0);
  }
  ...
  if (reactor.set_use_32bit_paging) try { reactor.set_use_32bit_paging(use_32bit_paging); } catch {}
  if (reactor.set_use_multilevel_paging) try { reactor.set_use_multilevel_paging(use_multilevel_paging); } catch {}
  if (reactor.set_shared_page_table_vaddr && shared_page_table_vaddr !== undefined) try { reactor.set_shared_page_table_vaddr(shared_page_table_vaddr); } catch {}
  if (reactor.set_shared_security_directory_vaddr && shared_security_directory_vaddr !== undefined) try { reactor.set_shared_security_directory_vaddr(shared_security_directory_vaddr); } catch {}
  ...
  try {
    if (useJit && reactor.jit_run) {
      ...
      if (r && typeof r.then === 'function') await r;
    } else throw new Error('No interp or jit_run on Reactor');
